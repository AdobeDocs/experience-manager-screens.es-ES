...
1. Vaya a [Descargas del reproductor AEM Screens](https://download.macromedia.com/screens/) para descargar Tizen Player.
...
Puede asignar un nombre de dispositivo fácil de usar al reproductor Tizen y, de este modo, enviar el nombre de dispositivo asignado a Adobe Experience Manager AEM (). Esta capacidad no solo le permite asignar un nombre a su reproductor Tizen, sino que también le permite asignar fácilmente el contenido adecuado.
...
1. El Reproductor de AEM Screens ahora debe instalarse e iniciarse automáticamente en su dispositivo Samsung.
...
>Hay algunos motores de explorador que son incompatibles con el *SameSite=None* AEM AEM atributo utilizado en el token de inicio de sesión emitido por la versión 6.5 a la versión 6.7 de la. Normalmente, el problema se puede resolver actualizando el explorador a la última versión disponible. En algunos casos, estas actualizaciones pueden no ser posibles, como con pantallas inteligentes, descodificadores u otros dispositivos con motores de exploración integrados.
...
1. Debería ver una nueva opción *Agentes de usuario que deben quedar exentos del atributo samesite*. Rellene esto con una regex correspondiente al agente de usuario que sea incompatible con el *SameSite=None* atributo.
   >[!NOTE]
   >Consulte [SameSite=None: Clientes incompatibles conocidos](https://www.chromium.org/updates/same-site/incompatible-clients) para obtener más información. Para el reproductor Tizen, utilice la regex: `(.*)Tizen(.*)`.
...
El aprovisionamiento remoto de Tizen Player le permite implementar cientos y miles de pantallas Samsung Tizen sin mucho esfuerzo. Evita el tedioso esfuerzo manual de configurar cada reproductor con la URL del servidor y el código de registro masivo, u otros parámetros y en el caso de Pantallas as a Cloud Service para configurar el modo de nube y el token de nube.
...
1. Configure TLS si es necesario. Vaya al puerto, seleccione el número de puerto del servidor y haga clic en **Guardar**.

1. Vaya a **Dispositivo** y compruebe el dispositivo que acaba de configurar. Una vez encontrado un dispositivo, haga clic en la casilla de verificación y seleccione **Aprobar**.
...
1. Una vez aprobado el dispositivo, debe aparecer en la lista de dispositivos. Haga clic en *Información* botón ubicado en la caja del dispositivo, es decir, **i**, como se muestra en la figura siguiente.

   >![imagen](/help/user-guide/assets/tizen/rms-6.png)

1. Aparece el cuadro de diálogo información del dispositivo. Seleccione el **Información del dispositivo** y haga clic en **Editar**.

   >![imagen](/help/user-guide/assets/tizen/rms-5.png)

1. Edite las opciones del dispositivo y seleccione **Configurar** pestaña. Vaya a **Lanzador de URL** e introduzca la dirección URL que aloja el wgt y `SSSP config file` para instalar un `SSSP` aplicación, como se muestra en la figura siguiente.
